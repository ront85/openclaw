...
const TIER1_PATTERNS: PatternSpec[] = [
  ...
  // Mistral
  {
    ...
    confidence: "tier1",
    minLength: 32,
    validate: (match) => /^[A-Za-z0-9]{32}$/.test(match) && match.startsWith("mistr"),
  },
  // Cohere
  {
    ...
    minLength: 40,
    validate: (match) =>
      match.startsWith("co-") || (/^[A-Za-z0-9]{40}$/.test(match) && match.startsWith("coh")),
  },
  // Deepseek
  {
  ...
  // Together AI
  {
    ...
    confidence: "tier1",
    minLength: 64,
    validate: (match) => /^[a-f0-9]{64}$/.test(match),
  },
  // Fireworks AI
  {
    pattern: /\bfw_[A-Za-z0-9]{32,}\b/g,
    provider: "FIREWORKS",
    confidence: "tier1",
    minLength: 35,
  },
...
function shouldSkipScan(text: string): boolean {
  ...
  if (
    !/\b(key|token|secret|password|auth|api|bearer|sk-|ghp_|xox|AIza|gsk_|pplx-|hf_|AKIA|fw_)\b/i.test(
      text,
    )
  ) {
    return true;
