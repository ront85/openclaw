...
describe("api-key-detector", () => {
  ...
  describe("inferProvider", () => {
    ...
    it("detects Fireworks AI keys", () => {
      expect(inferProvider("fw_abc123")).toBe("FIREWORKS");
    });
  ...
  describe("detectApiKeys - Tier 1", () => {
    ...
    it("detects Fireworks AI keys", () => {
      const text = "My key is fw_" + "A".repeat(32);
      const detected = detectApiKeys(text, DEFAULT_CONFIG);

      expect(detected.length).toBeGreaterThanOrEqual(1);
      const fwKey = detected.find((k) => k.provider === "FIREWORKS");
